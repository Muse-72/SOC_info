...
//                  SPFD5408/1505/B505/C505/NT35310/NT35510/SSD1963/ILI9481等		    
//正点原子@ALIENTEK
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	LCD_SetCursor(x,y);	    
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0x9481)LCD_WR_REG(0X2E);//9341/6804/3510/1963 发送读GRAM指令
	...
	opt_delay(2);	  
 	...
 	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0x9481)		//9341/NT35310/NT35510要分2次读出
 	{
		opt_delay(2);	  
		b=LCD_RD_DATA(); 
...
{					   
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0x9481)LCD_WR_REG(0X29);	//开启显示
...
{	   
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0x9481)LCD_WR_REG(0X28);	//关闭显示
...
void LCD_Scan_Dir(u8 dir)
{
	...
	if(lcddev.id==0x9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963||lcddev.id==0x9481)//9341/6804/5310/5510/1963,特殊处理
	{
		switch(dir)
		{
...
void LCD_Display_Dir(u8 dir)
{
	if(dir==0)			//竖屏
	{
		...
		lcddev.width=240;
		lcddev.height=320;
		if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310 ||lcddev.id==0x9481)
		{
			lcddev.wramcmd=0X2C;
	 		lcddev.setxcmd=0X2A;
			lcddev.setycmd=0X2B;  	 
			if(lcddev.id==0X6804||lcddev.id==0X5310 ||lcddev.id==0x9481)
			{
				lcddev.width=320;
				lcddev.height=480;
			}
...
{    
	...
	twidth=sx+width-1;
	theight=sy+height-1;
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X6804 ||lcddev.id==0x9481 ||(lcddev.dir==1&&lcddev.id==0X1963))
	{
		LCD_WR_REG(lcddev.setxcmd); 
		LCD_WR_DATA(sx>>8); 
		LCD_WR_DATA(sx&0XFF);	 
...
//				temp|=pic[tem];
//				tem++;
//				LCD_Fast_DrawPoint(i,j,temp);
//				
//			}
//		}
//	
//}
void LCD_DrawPicture(u16 x,u16 y, u16 pic_H, u16 pic_V, const u8* pic) //显示图片
{
	u32 i=0,j=0,temp;
	...
	for(j=0; j<pic_V; j++)
	{
		...
		for(i=0; i<pic_H; i++)
		{
			temp  = pic[j*pic_H*2+i*2+1];	temp <<= 8;
			temp += pic[j*pic_H*2+i*2];
//			LCD->LCD_RAM = temp;
			LCD_WR_DATA(temp);//写入数据 
		}	
	}
}
